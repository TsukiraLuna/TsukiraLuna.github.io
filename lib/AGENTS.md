...
9. **按行切分内容时必须兼容 CRLF** —— 仓库已声明 `.gitattributes`（`* text=auto eol=lf`），检出时会把内容统一成 LF，正常情况碰不到 CRLF。但**来自外部的内容**仍可能带 CRLF —— 从别处拷进来的文章、编辑器改写、`core.autocrlf` 配置过旧的机器，管线必须容忍。而 JS 的 `.` 不匹配行终止符、`\r` 正是行终止符，所以 `/^(#{1,6})\s+(.+)$/` 这类正则一旦遇到残留的结尾 `\r` 就会整体失配。真实后果：**文章的目录会静默变空**（正文照常渲染，只有目录消失，极难察觉）。凡按行处理内容，一律用 `split(/\r?\n/)`，不要用 `split("\n")`。`lib/content.test.ts` 有对应的 CRLF 回归护栏。
