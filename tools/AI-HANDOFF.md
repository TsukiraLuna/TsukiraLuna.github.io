...
> | 文件 | 定位 | 何时读 |
> |---|---|---|
> | **根目录 `AGENTS.md`** | **入口，AI 会自动加载** | 永远先读它 |
> | 本文件 | 详细版，补充入口没写的背景 | 需要更细的说明时 |
> | `tools/series-convention.md` | 系列文章约定 | 发系列章节前 |
> | `tools/latex-to-blog.md` | LaTeX 转化流程 | 转讲义前 |
> | 各目录 `AGENTS.md` / `CLAUDE.md` / `DESIGN.md` | 模板官方规则，优先级最高 | 改对应目录前 |
>
...
> ⚠️ 与根目录 `AGENTS.md` 有部分重叠是**刻意**的：那份是启动即读的速查，
> 这份保留更完整的背景与教训。改动一处时请顺手检查另一处是否需要同步。
